...
export default defineEventHandler(async (event) => {
  ...
  if (body.published_year !== undefined) {
    const currentYear = new Date().getFullYear()
    if (body.published_year === null || body.published_year === '') {
  ...
  try {
    ...
    const updatedBook = await db.updateBook(id, {
      ...
      ...(body.author_id !== undefined && { author_id: parseInt(body.author_id) }),
      ...(body.isbn !== undefined && { isbn: body.isbn.trim() }),
      ...(body.published_year !== undefined && { published_year: parseInt(body.published_year) }),
      ...(body.status !== undefined && { status: body.status.toUpperCase() as 'AVAILABLE' | 'LOANED' | 'RESERVED' }),
